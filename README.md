# ad-pipeline — ad analytics ETL with a production-failure simulator

![CI](https://github.com/Aman12x/ad-pipeline/actions/workflows/ci.yml/badge.svg)

An end-to-end ad analytics pipeline — **raw API data → validated warehouse →
budget decisions** — built against a simulator that fakes the Google Ads and
Meta APIs, including the failure modes that break real ad pipelines. Clone and
run in two minutes; **no ad accounts or credentials required**.

Most portfolio ETL projects load a static CSV. Real ad pipelines are hard for
different reasons, and this project simulates and handles all of them:
...
# terminal 1 — the fake ad platforms
uvicorn simulator.app:app --port 8787

# terminal 2 — run the ETL, then explore
python -m pipeline.run                  # defaults to as-of today
python -m pipeline.run --as-of 2026-08-01   # replay any past day
streamlit run dashboard/app.py          # dashboard on :8501
python scripts/export_report.py         # static PNG report -> assets/
pytest                                  # 10 tests, ~10s
```
